...
class Room:
	...
	def EnergyReport(self,StartDate):
		...
		for metricreport in results['metricReportList']:
		    ...
		    requeteRoom  ="INSERT INTO LightEnergyQuarterOld VALUE (NULL,'{0}',{1},{2},{3},{4},{5},{6},{7})"
...
AllBuilding=Room(5,'00000000-0000-0002-0000-000000000000');

Now = datetime.now()
StartDate = datetime(2017,11,01,0,0,0,0)
while(StartDate<Now):
	
	RoomTesla.EnergyReport(StartDate)
	RoomLumiere.EnergyReport(StartDate);
	RoomNobel.EnergyReport(StartDate);
	RoomTuring.EnergyReport(StartDate);
	AllBuilding.EnergyReport(StartDate);
